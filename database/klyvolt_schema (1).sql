...
CREATE TABLE usuario (
    ...
    ID_empresa      INT NOT NULL REFERENCES empresa(IDempresa),
...
CREATE TABLE setor (
    ...
    ID_usuario      INT NOT NULL REFERENCES usuario(IDusuario)
);
...
CREATE TABLE maquina (
    ...
    ID_setor            INT NOT NULL REFERENCES setor(IDsetor)
);
...
CREATE TABLE sensor (
    ...
    ID_maquina              INT NOT NULL REFERENCES maquina(IDmaquina),
    CONSTRAINT chk_status_conexao CHECK (status_conexao IN ('online','offline')),
...
CREATE TABLE leitura (
    ...
    ID_usuario          INT REFERENCES usuario(IDusuario),
    ID_consumo          INT REFERENCES consumo(IDconsumo),
...
CREATE TABLE tomada_decisao (
    ...
    data_tomada_decisao TIMESTAMP NOT NULL,
    valor_referencia    DECIMAL(12,2) NOT NULL,
    status_execucao     VARCHAR(15) NOT NULL,
    observacao          TEXT,
    ID_decisao          INT NOT NULL REFERENCES decisao(ID_decisao),
    ID_leitura          INT REFERENCES leitura(IDleitura),
    ID_consumo          INT REFERENCES consumo(IDconsumo),
    CONSTRAINT chk_status_execucao CHECK (status_execucao IN ('pendente','executada','cancelada'))
);
